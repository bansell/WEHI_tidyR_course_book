...
### Reordering gene plots {#reorder_go_plot}

The y axis order of the gene family [bar chart](#bar-chart) that we created in [week 3](#week3), can be improved using a similar approach.

First if `complete_table` is not currently in your R Environment, you could re-run the week 3 code to generate this data frame. Alternatively, download complete_table.tsv from [this folder](https://www.dropbox.com/sh/hihrdes1gom7e9h/AABKerUadszjBANjYXcPaTaVa?dl=0) into your Desktop WEHI_tidyR_course/data_files folder, and import the table using read_tsv():

```{r, eval=FALSE}
complete_table <- read_tsv('~/Desktop/WEHI_tidyR_course/data_files/complete_table.tsv')
```

```{r, eval=TRUE,include=FALSE,echo=FALSE}
complete_table <- read_tsv('data_files/complete_table.tsv')
```
